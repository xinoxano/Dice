...
@implementation HNEYACYAMLKeyedArchiver {
    NSMutableData *_dataForWriting;
    FILE *_fileForWriting;
    
    BOOL _scalarAnchorsAllowed;
    
    HNEYACYAMLArchivingObject *_rootObject;
...
+ (NSData *)archivedDataWithRootObject:(id)rootObject options:(YACYAMLKeyedArchiverOptions)options
{
    ...
    HNEYACYAMLKeyedArchiver *archiver = [[[self class] alloc] initForWritingWithMutableData:ret options:options];
...
{
    BOOL success = NO;
    HNEYACYAMLKeyedArchiver *archiver = [[[self class] alloc] initForWritingToFile:path options:options];
    if(archiver) {
        [archiver encodeRootObject:rootObject];
        [archiver finishEncoding];
        success = YES;
...
{
    _rootObject = [[HNEYACYAMLArchivingObject alloc] initWithRepresentedObject:nil
                                                                forArchiver:self];
...
{
    return (__bridge HNEYACYAMLArchivingObject *)CFDictionaryGetValue(_archivedObjectToItem, (__bridge void *)object);
}
