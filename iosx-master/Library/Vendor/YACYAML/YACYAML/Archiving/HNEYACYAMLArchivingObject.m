...
#import "HNEYACYAMLArchivingObject.h"
#import "YACYAMLArchivingExtensions.h"

#import "YACYAMLConstants.h"

@interface HNEYACYAMLArchivingObject ()
@property (nonatomic, assign) BOOL needsAnchor;
@end

@implementation HNEYACYAMLArchivingObject {
    __unsafe_unretained HNEYACYAMLKeyedArchiver *_archiver;
...
- (HNEYACYAMLArchivingObject *)_archivingObjectForObject:(id)obj
{
    HNEYACYAMLArchivingObject *archivingObject = [_archiver previouslySeenArchivingObjectForObject:obj];
    if(archivingObject) {
        ...
        archivingObject = [[HNEYACYAMLArchivingObject alloc] initWithRepresentedObject:obj
...
- (void)encodeChild:(id)obj forKey:(id)key
{
    ...
    HNEYACYAMLArchivingObject *archivingObject = [self _archivingObjectForObject:obj];
    if(key) {
        if(!_keyedChildren) {
    ...
    if(obj) {
        if(_emittedAnchor) {
            ...
            if([obj respondsToSelector:@selector(YACYAMLScalarString)]) {
                ...
                NSString *string = [(id<HNEYACYAMLArchivingCustomEncoding>)obj YACYAMLScalarString];
                ...
                if(string) {
                    ...
                    if(stringCharsLength) {
                        if(!customTag &&
                           [HNEYACYAMLKeyedUnarchiver classForYAMLScalarString:string]) {
                ...
                if(_unkeyedChildren || !_keyedChildren) {
                    ...
                    for(HNEYACYAMLArchivingObject *child in (__bridge NSArray *)_unkeyedChildren) {
                ...
                if(_keyedChildren) {
                    ...
                    for(HNEYACYAMLArchivingObject *key in (__bridge NSArray *)_keyedChildren) {
