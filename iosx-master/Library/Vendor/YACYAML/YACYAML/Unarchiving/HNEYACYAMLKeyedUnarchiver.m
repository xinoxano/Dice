...
@implementation HNEYACYAMLKeyedUnarchiver {
...
+ (void)initialize
{
    if(self == [HNEYACYAMLKeyedUnarchiver class]) {
...
- (BOOL)_setupForReadingWithOptions:(YACYAMLKeyedUnarchiverOptions)options
{
    ...
    if(success) {
        HNEYACYAMLUnarchivingObject *obj = [[HNEYACYAMLUnarchivingObject alloc] initWithParser:&_parser
                                                                           forUnarchiver:self];
        if (obj) {
